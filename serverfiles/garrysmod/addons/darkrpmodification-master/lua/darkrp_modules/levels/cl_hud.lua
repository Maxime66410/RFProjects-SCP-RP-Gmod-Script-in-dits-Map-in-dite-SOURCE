	...
	if(LevelSystemConfiguration.DisplayLevel) then
		for k, ply in pairs(players or player.GetAll()) do
			...
			if GAMEMODE.Config.globalshow and ply ~= localplayer then
					...
					pos = pos:ToScreen()
					pos.y = pos.y-20
					draw.DrawText('Niveau : '..(ply:getDarkRPVar('level') or 0), "DarkRPHUD2", pos.x+1, pos.y -56, Color(0,0,0,255), 1)
					draw.DrawText('Niveau : '..(ply:getDarkRPVar('level') or 0), "DarkRPHUD2", pos.x, pos.y -55, Color(255,255,255,200), 1)
...
local function HUDPaint()
	...
	// Render the text
	draw.DrawText('Niveau '..(LocalPlayer():getDarkRPVar('level') or 0)..' - '..percent2 ..'%', "HeadBar", ScrW()/2,7+(LevelSystemConfiguration.XPBarYPos or 0),(LevelSystemConfiguration.XPTextColor or Color(255,255,255,255)), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
